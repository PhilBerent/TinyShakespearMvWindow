...
file_path = 'C:\\Users\\Phil\\Documents\\AI\\Code\\GPT From Scratch Karpathy\\input.txt'
...
class SlidingWindowMatrixMult3D(nn.Module):
    ...
    def forward(self, Q, K):
        ...
        kwBias2 = self.keyWindowBias.permute(0, 1, 3, 2)
        K_sw2 = K.unfold(1, T, 1).permute(0, 1, 3, 2)
        K_sw2 = K_sw2 + kwBias2
        
        # Compute the Z matrix using einsum
        Z2 = torch.einsum('bwc,bwtc->bwt', Q, K_sw2)
...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        ...
        self.keyICR = nn.Linear(n_embd, head_size, bias=True)
        self.keyCR = nn.Linear(n_embd, head_size, bias=True)
        self.vICR = nn.Linear(n_embd, head_size, bias=True)
        self.vCR = nn.Linear(n_embd, head_size, bias=True)
        ...
        self.wSoftMaxMult = nn.Parameter(torch.ones(1, block_size)*.5)
    ...
    def forward(self, x):
        ...
        wgtAdj =self.wSoftMaxMult.view(1,1,-1)
        weiNew = weiNew * wgtAdj
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        xpred = xNorm[:,-block_size:,:]
        xPastNorm = xNorm[:,:block_size-1,:]
        ...
        xout = torch.cat((xPastNorm, xtemp4), dim=1)
        block_layer += 1
        return xout
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.prevMult = nn.Parameter(torch.ones(n_layer))
    ...
    def forward(self, idx, targets=None):
        ...
        for i, block in enumerate(self.blocks):  # Iterate through blocks
            ...
            prevMult = self.prevMult[i]
            totalMult = prevMult + 1
            x = (x + xPrev*prevMult)/totalMult
